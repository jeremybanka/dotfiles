...
def build-guest-script [dry_run: bool] {
  ...
  [
    ...
    ''
    'dirty_space_active() {'
    '  # The cleanup script itself mentions scrubs-dirty paths, so matching the'
    '  # full ps command line would make this guard permanently true.'
    '  for proc_dir in /proc/[0-9]*; do'
    '    [ "$proc_dir" != "/proc/$$" ] || continue'
    ''
    ...
    '      if tr "\000" "\n" < "$proc_dir/cmdline" 2>/dev/null | grep -Eq "(^|/)(dirty-exec\\.sh|scrubs-dirty-exec)$"; then'
    '        return 0'
    '      fi'
    '    fi'
    '  done'
