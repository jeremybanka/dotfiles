...
enum HelixThemeAgentError: Error, CustomStringConvertible {
	...
	var description: String {
		...
		case .invalidArguments:
			...
			"""
		case let .missingHelixConfig(path):
			return "Missing Helix config: \(path)"
		case let .refusingToReplaceNonSymlink(path):
...
private final class ThemeAgent {
	...
	init(homeDirectory: URL) {
		...
		self.helixConfigDirectory = homeDirectory
	...
	func syncHelixTheme() throws -> AppearanceMode {
		...
		try fileManager.createSymbolicLink(atPath: activeConfigPath.path, withDestinationPath: sourceName)
	...
	func installLaunchAgent() throws -> String {
		...
		let logsDirectory = homeDirectory
			.appendingPathComponent("Library", isDirectory: true)
