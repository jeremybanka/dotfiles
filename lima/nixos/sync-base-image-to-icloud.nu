...
def main [
  ...
  let local_dir = ($env.SCRUBS_LOCAL_BASE_IMAGE_DIR? | default ($repo_root | path join "lima" "nixos" "qcow2"))
  ...
  let source_path = ($local_dir | path join $image_name)
  let dest_path = ($icloud_dir | path join $image_name)

  if not ($source_path | path exists) {
    error make { msg: $"Local base image not found: ($source_path)" }
  }
